...
/**
 * \file
 *         Digital I/O abstraction (platform specific part: Windows).
 *
 * \author
...
/******************************************************************************/
/**
 * \typedef xme_hal_dio_arch_portState_t
 *
...
xme_hal_dio_getPortConfiguration(xme_hal_io_port_t port, xme_hal_dio_portConfiguration_t* configuration)
{
	...
	XME_CHECK(port < XME_HAL_DIO_NUM_PORTS, XME_CORE_STATUS_INVALID_PARAMETER);
...
{
	XME_ASSERT(NULL != configuration);
	XME_CHECK(portPin.port < XME_HAL_DIO_NUM_PORTS, XME_CORE_STATUS_INVALID_PARAMETER);
...
xme_hal_dio_setPort(xme_hal_io_port_t port, xme_hal_dio_portState_t state)
{
	...
	XME_LOG(XME_LOG_VERBOSE, "Setting DIO port #%u.%u value to 0x%02X\n", port, state);
	return XME_CORE_STATUS_SUCCESS;
}
