...
#define XME_HAL_TABLE_INIT(name) \
	do { (name).count = 0; (name).maxHandle = XME_HAL_TABLE_INVALID_ROW_HANDLE; (name).power = 0; (name).array = NULL; } while (0)
 ...
 *         table to fit the new item. On systems with static memory allocation,
 *         it will use the next free slot and return a handle to that slot.
 *
...
#define XME_HAL_TABLE_ITEM_FROM_HANDLE(name, handle) \
	((handle > XME_HAL_TABLE_INVALID_ROW_HANDLE && handle <= (name).maxHandle) ? (name).array[handle-1] : NULL)
...
#define XME_HAL_TABLE_ITERATE_BEGIN(name, loopHandleType, loopHandle, loopItemType, loopItem) \
	{ \
		...
		{ \
			...
			if (NULL != (loopItem = (name).array[loopHandle-1])) \
			{
...
#define XME_HAL_TABLE_GET_NEXT(name, loopHandleType, loopHandle, loopItemType, loopItem, condition) \
	...
	{ \
		...
		{ \
			...
			if (loopHandle <= (loopHandleType)(name).maxHandle && NULL != (loopItem = (name).array[loopHandle-1])) \
			{ \
				if (condition) \
				{ \
					break; \
		...
		{ \
			...
			{ \
				...
				{ \
					...
					{ \
						compareItem1 = (compareType *)loopItem1 + offset; \
						compareItem2 = (compareType *)loopItem2 + offset; \
