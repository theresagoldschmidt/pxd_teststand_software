...
xme_prim_ipLoginClientProxy_create(xme_prim_ipLoginClientProxy_configStruct_t* config)
{
	...
	config->managementChannelsToNewNode = XME_CORE_DCC_INVALID_SUBSCRIPTION_HANDLE;
	...
	(
		...
		(
			config->loginServerSocket =
				...
				(
					xme_core_interface_getInterface(config->interfaceId),
	...
	XME_CHECK_REC
	(
		XME_CORE_STATUS_SUCCESS == xme_hal_net_openSocket(config->loginServerSocket),
...
xme_prim_ipLoginClientProxy_activate(xme_prim_ipLoginClientProxy_configStruct_t* config)
{
	...
	XME_CHECK
	(
		XME_CORE_STATUS_SUCCESS == xme_hal_sched_setTaskExecutionState(config->loginTaskHandle, true),
...
xme_prim_ipLoginClientProxy_deactivate(xme_prim_ipLoginClientProxy_configStruct_t* config)
{
	...
	XME_CHECK
	(
		XME_CORE_STATUS_SUCCESS == xme_hal_sched_setTaskExecutionState(config->loginTaskHandle, false),
	);
}
...
xme_prim_ipLoginClientProxy_destroy(xme_prim_ipLoginClientProxy_configStruct_t* config)
{
	...
	{
		xme_core_status_t rval = xme_core_dcc_unpublishTopic(config->managementChannelsToNewNode);
...
xme_prim_ipLoginClientProxy_taskCheckLoginRequests(void* userData)
{
	...
	while (XME_CORE_STATUS_SUCCESS == xme_hal_net_selectSocket(config->loginServerSocket, true, false, 1))
	{
		// A request has been received
		xme_core_packet_loginRequest_t loginRequest;
		requestServed = true;
		...
		if (sizeof(loginRequest) == xme_hal_net_readSocket(config->loginServerSocket, &loginRequest, sizeof(loginRequest)))
		{
			...
			if (!XME_CORE_PACKET_VALID(loginRequest))
			{
				continue;
			}

			// Ensure that the packet contains a login request
			if (XME_CORE_PACKET_HEADER_TYPE_LOGINREQUEST != XME_CORE_PACKET_TYPE(loginRequest))
			{
				continue;
			}
			...
			XME_ASSERT_NORVAL(XME_CORE_INTERFACEMANAGER_INVALID_INTERFACE_ID != loginRequest.data.edgeNodeInterfaceId);
...
)
{
	...
	(
		XME_LOG_VERBOSE,
		"Forwarding login response to peer (DT=0x%08X%08X, GUID=0x%08X%08X, N=%d, E=%d:%d, AC=%d, MC=%d)\n",
	...
	// Forward the response to new node
	{
		...
		XME_CORE_PACKET_INIT(loginResponsePacket, XME_CORE_PACKET_HEADER_TYPE_LOGINRESPONSE);
	...
	// Notify directory to set up management channels for new node
	{
		...
		// Publish publication
		{
			xme_core_status_t rval = xme_core_dcc_sendTopicData(config->managementChannelsToNewNode, &managementChannels, sizeof(managementChannels));
			XME_ASSERT_NORVAL(XME_CORE_STATUS_SUCCESS == rval);
		}
	}
}
