...
)
{
...

	{
		...
		(
			...
			(uint32_t)(XME_CORE_PACKET_PAYLOAD(loginRequestPacket).deviceType >> 32), (uint32_t)(XME_CORE_PACKET_PAYLOAD(loginRequestPacket).deviceType & 0xFFFFFFFF),
		...
		else
		{
			...
			(
				...
				xme_prim_ipLoginServerProxy_pendingResponseItem_t, item,
				{
					...
					{
						if (xme_hal_time_getInterval(&item->lastUpdate, false) > item->responseTimeoutMs)
						{
	...
	if (XME_CORE_STATUS_SUCCESS == xme_hal_net_selectSocket(config->loginClientSocket, true, false, 100))
	{
		...
		)
		{
			...
			(
				...
				xme_prim_ipLoginServerProxy_pendingResponseItem_t, item,
				{
					if (item->deviceType == XME_CORE_PACKET_PAYLOAD(loginResponsePacket).deviceType && item->deviceGuid == loginResponsePacket.data.deviceGuid)
					{
