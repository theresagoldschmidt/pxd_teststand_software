...
// TODO: Replace with XME specific define
#if !NO_SYS
#include "FreeRTOS.h"
#endif

/* Priorities at which the tasks are created. */
#define InterruptTaskPriority ( tskIDLE_PRIORITY + 1 )
//*****************************************************************************
//
...
//*****************************************************************************
#if !NO_SYS
static signed char g_pcQueueMem[sizeof(xSemaphoreHandle)];
#endif
...
lwIPServiceTimers(void)
{
    ...
    if((g_ulLocalTimer - g_ulHostTimer) >= HOST_TMR_INTERVAL)
    {
        ...
        // Optional: Call the application-supplied host timer callback function.
    }
#endif
...
lwIPPrivateInit(void *pvArg)
{
	...
	xTaskCreate(
			lwIPInterruptTask,
			(signed portCHAR *)"eth_int",
			128,
            NULL ,
            InterruptTaskPriority,
            NULL);
#endif
...
void ETH_IRQHandler(void)
{
    ...
    //
    // Optional: Read and Clear the interrupt.
    //
