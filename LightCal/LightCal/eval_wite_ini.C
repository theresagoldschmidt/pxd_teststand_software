...
if(parameter2[1]<0)	cout<<"fit falling funct"<<endl;
		
		
		 
		range_low=bisec(graph[j],lower_limit,fall[j]);
		range_high=bisec(graph[j],upper_limit,fall[j]);
...
for( int i=0; i<num_files;i++){
	...
	for( int j=0; j<5;j++){
	    ...
	    calli+=offset;
        calli+="=";	
        calli+=-1.0*param[i][j][1]*10000/TMath::Abs(param[i][j][0]); // divede by gain 
	    myfile << calli.Data()<<endl;
