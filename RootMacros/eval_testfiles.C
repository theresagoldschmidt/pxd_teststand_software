...
TString path="c:/root/calibration/";
...
upper_limit=1*range+comp_y_min;
lower_limit=0.0*range+comp_y_min;
...
graph[j]->Fit(lin[j],"Q");
	...
	res_graph[j]->SetName(string.Data());
			...
			for(int k=1;k<(graph[j]->GetN()-1);k++){
					...
					if(j<=2)	res_graph[j]->SetPoint(k-1,x,x-y); // #TMath::Abs(x)-TMath::Abs(y));  //resi);
					if(j>=3)	res_graph[j]->SetPoint(k-1,x,TMath::Abs(x)-TMath::Abs(y));  
					}
...
TString filename="C:/root/Callibration/test_constants_node_";
filename+=node;
filename+=".ini";
...
for( int i=0; i<num_files;i++){
	...
	for( int j=0; j<5;j++){
	    ...
	    calli+=offset;
        calli+="=";	
        calli+=1.0*param[i][j][1]; // mult by 100 to fit into firmware
	    myfile << calli.Data()<<endl;
