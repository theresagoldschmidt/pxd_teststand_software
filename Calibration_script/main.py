...
# plot
plt.scatter(x_2,y_2, color='grey', marker='x')
plt.plot(x_2, m_2 * x_2 + b_2, color='r' )
plt.xlabel('$U_{out}$ [mV]')
plt.ylabel('$U_{load}$ [mV]')
plt.title('$U_{out} vs. U_{load}$')
plt.text(1000,200, f'slope = {round(m_2, 4)}, int= {round(b_2,4)}')
...
# plot
plt.scatter(x_3,y_3, color='grey', marker='x')
plt.plot(x_3, m_3 * x_3 + b_3, color='r' )
...
""" 
PLOTS (3)_______________________________________________________________________________________________data_IvsI = pd.read_csv(config["calibration_data"].get("data_path"),sep=" ",header=None)
"""
...
# plot
plt.scatter(x_4,y_4, color='grey', marker='x')
plt.plot(x_4, m_4 * x_4 + b_4, color='r' )
plt.xlabel('$Limit DAC$ [mV]')
plt.ylabel('$Limit Current$ [mA]')
plt.title('$DAC LIMIT vs. I Measured$')
plt.text(0,-1500, f'slope = {round(m_4, 4)}, int= {round(b_4,4)}')
